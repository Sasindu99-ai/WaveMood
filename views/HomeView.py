...
class HomeView(View):
    ...
    def __init__(self, parent: Optional[WindowType] = None):
        ...
        try:
            ...
            try:
                ...
                # if this also fails, leave the value and let _saveRecording handle errors
                pass
    ...
    def _onSelectFile(self):
        ...
        if filePath:
            ...
            # Show the loading indicator (UI thread), then start the background job
    ...
    def _onWaveformLoaded(self, success, file_path):
        ...
        if success:
            ...
            # update the current audio file so playback uses the latest selected/recorded file
    ...
    def _onStopRecording(self):
        ...
        # wait briefly for the recording thread to consume remaining frames
        if self._recordingThread and self._recordingThread.is_alive():
            ...
            # after the join attempt, make a final pass to drain queue into buffer
            try:
                while True:
                ...
                pass
        # next: save (runs on the main thread)
        self._saveRecording()
    ...
    def _recordAudio(self):
        def callback(indata, _, _time, status):
        ...
        try:
            ...
            # record errors should not crash the app; update the status on the main thread if possible
            print("InputStream error:", _e)
    ...
    def _saveRecording(self):
        ...
        # Clip to a valid range and convert to int16
        ...
        # Load the waveform for the newly recorded audio (start loading in the background)
        self.playbackStatus.setText("Loading waveform...")
        self.waveformWidget.show_loading()
        threadPool.start(
            self.waveformWidget.load_waveform,
    ...
    def _pausePlayback(self):
        ...
        # stop VU updates and clear the display
        try:
            self._vuTimer.stop()
            with self._vu_lock:
                self._vu_level = 0.0
    ...
    def _stopPlayback(self):
        ...
        self._playbackUpdateTimer.stop()
        # reset the waveform cursor to start
        ...
        # Stop and close the stream if it exists and reset the frame index
        if self._playbackStream:
            try:
                self._playbackStream.stop()
            except Exception:
    ...
    def _playAudio(self, file_path, start_frame: int = 0):
        try:
            ...
            def callback(outdata, frames, _time, _):
                ...
                # update the shared frame index and playback position
                frame_index += to_copy
                with self._playbackFrameLock:
                    self._playbackFrameIndex = frame_index
            ...
            # VU timer is managed from the main thread (_startPlayback/_pause/_stop)

            # keep running while the stream is active and the playing flag may be toggled by pause/stop
            # sounddevice handles the callback; return when the stream is stopped/closed
    ...
    def _autoStopPlayback(self):
        ...
        # fully reset playback to the initial state so UI and cursor return to start
        with self._playbackFrameLock:
            self._playbackFrameIndex = 0
        self._playbackPosition = 0
    ...
    @staticmethod
    def _estimate_f0_autocorr(frame, sr, fmin=50, fmax=800):
    ...
    def _ensure_arduino_connected(self):
        ...
        if not self._arduino_connected:
            try:
                ...
                self._arduino_connected = False
                self.playbackStatus.setText(f"Arduino connection failed: {_e}")
                return False
        return True
    ...
    def _send_emotion_to_arduino(self, emotion):
        ...
        try:
            ...
            self.playbackStatus.setText(f"Error sending to Arduino: {_e}")
    ...
    def _onAnalysisComplete(self, res, file_path):
        ...
        try:
            ...
            logger.error(f"Error in _onAnalysisComplete: {_e}")
    ...
    def _onModelSelected(self, model: Model):
        """Mark the selected model button as active (radio behavior) and store the active model."""
        try:
